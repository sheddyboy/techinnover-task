...
const AddTaskForm = ({}: AddTaskFormProps) => {
  const queryClient = getQueryClient();
  const { currentCategory, setCreateTaskModal } = useStore();
  ...
  function onSubmit(values: z.infer<typeof addTaskFormSchema>) {
    ...
    toast.promise(addTask(values), {
      ...
      success: async ({ data, error }) => {
        ...
        setCreateTaskModal({ showModal: false, category: currentCategory });
        return `Task created`;
      },
      error: (error) => {
